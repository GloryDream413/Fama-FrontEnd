...
export default function DataContent({drawdown}) {
  const { data: dataCurrentCoin, error: errGetCurrentCoin, isLoading: isGetCurrentCoinLoading } =
  useContractRead({
    ...wagmiContractConfig,
    functionName: 'getCurrentCoin',
    chainId: 5,
  })

  if(String(dataCurrentCoin) == "usdc")
  {
    ...
    const totalUSD = (Number(dataTotalVaultUSDC) /1000000) * (Number(usdcPrice) /100000000);
  ...
  else
  {
    ...
    const totalUSD = (Number(dataTotalVaultETH) / (10 ** 18)) * (Number(ethPrice) /100000000);
    console.log("TotalUSD=", Number(dataTotalVaultETH));
    const percentageUsd = totalUSD / 50000 * 100
